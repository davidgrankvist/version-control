...
namespace VersionControl.Test.Framework
{
    ...
    public static class CommandTestDataHelper
    {
        ...
        private static IEnumerable<(string[] Args, CommandWrapper Wrapper)> GetSuccessfulParseTestCasesAsTuples()
        {
            ...
            yield return (new string[] { "history", "from-change-id" }, new CommandWrapper(
                new HistoryCommand(changeService, new HistoryQuery("from-change-id", null, HistoryQueryFormat.Compact))));
            yield return (new string[] { "history", "from-change-id", "to-change-id" }, new CommandWrapper(
                new HistoryCommand(changeService, new HistoryQuery("from-change-id", "to-change-id", HistoryQueryFormat.Compact))));
            yield return (new string[] { "history", "from-change-id", "to-change-id", "--format", "compact" }, new CommandWrapper(
                new HistoryCommand(changeService, new HistoryQuery("from-change-id", "to-change-id", HistoryQueryFormat.Compact))));
            yield return (new string[] { "history", "from-change-id", "to-change-id", "-f", "compact" }, new CommandWrapper(
                new HistoryCommand(changeService, new HistoryQuery("from-change-id", "to-change-id", HistoryQueryFormat.Compact))));
